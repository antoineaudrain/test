...
export type CompletedDeliveryStopTableRow = {
  id: string;
  name: string;
  sequence: number;
  type: StopType;
...
const columnHelper = createColumnHelper<CompletedDeliveryStopTableRow>();

type CompletedDeliveryStopTableProps = {
  data: CompletedDeliveryStopTableRow[];
};

export function CompletedDeliveryStopTable({
  data,
}: CompletedDeliveryStopTableProps) {
      ...
      ...(hasDriverNotes
        ? [
            columnHelper.accessor("driverNotes", {
              ...
              cell: (props) => {
                if (props.row.original.status !== "FAILED") {
                  return null;
                }
        ...
        ? [
            columnHelper.accessor("imageUrl", {
              ...
              cell: (props) => {
                ...
                if (!raw) return <Text className="italic opacity-30">-</Text>;

                const uint8 = new Uint8Array(raw as unknown as ArrayBuffer);
  ...
  const table = useReactTable<CompletedDeliveryStopTableRow>({
