...
import { completeStop } from "@/features/stops/actions/mutations/completeStop";
import {
  type CompleteStopInput,
  CompleteStopSchema,
} from "@/features/stops/schemas/completeStop";

type CompleteStopButtonProps = {
  deliveryId: string;
  stopId: string;
};

export function CompleteStopButton({
  deliveryId,
  stopId,
}: CompleteStopButtonProps) {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<CompleteStopInput>({ resolver: zodResolver(CompleteStopSchema) });
  ...
  const handleComplete = handleSubmit(async (input) => {
    startTransition(async () => {
      ...
      await completeStop({ deliveryId, stopId, input });
      setIsOpen(false);
      resetForm();
    });
  });
  ...
  return (
    <>
      <Button
        ...
        className="rounded-l-none border-l-none flex-1"
      >
        <div className="py-1.5 flex flex-col items-center">
          <CheckIcon className="h-5 w-5 text-green-600" /> Livrer
        </div>
      </Button>
      ...
      >
        <form onSubmit={handleComplete}>
          ...
          <DialogDescription>
            Validez cette étape pour indiquer que la livraison a bien été
            effectuée.
          </DialogDescription>
