...
export function DeliveryCreationInterface({
  ...
  // Store initial deliveries snapshot for change detection
  ...
  // Detect if any changes have been made from initial state
  const hasUnsavedChanges = useMemo(() => {
    // Check if deliveries were deleted
    if (deletedDeliveryIds.length > 0) return true;

    // Check if new deliveries were added
    const hasNewDeliveries = deliveries.some((d) => !d.id);
    if (hasNewDeliveries) return true;

    // Check if count changed
    if (deliveries.length !== initialDeliveriesRef.current.length) return true;
    ...
    for (let i = 0; i < deliveries.length; i++) {
      ...
      if (
        ...
        current.vehicleId !== initial.vehicleId
      ) {
        return true;
      }

      // Compare endClientIds arrays
      if (current.endClientIds.length !== initial.endClientIds.length) {
        return true;
      }

      const currentIdsSorted = [...current.endClientIds].sort();
      const initialIdsSorted = [...initial.endClientIds].sort();
      if (currentIdsSorted.some((id, idx) => id !== initialIdsSorted[idx])) {
        return true;
      }
    }
    ...
    <>
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="flex flex-col max-sm:w-full sm:flex-1 gap-y-2">
          <Heading>Organisation des Livraisons</Heading>
          <Text>
